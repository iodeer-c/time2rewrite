...
- reference_window：引用其他时间节点做同期/对齐计算，例如 去年同期、上月同期
...
modifier 只可能是以下三类之一：
- none：没有额外修饰
- regular_grain：普通粒度，例如 每天、每月、每个月、各月、各月份、每季度
- calendar_selector：依赖外部日历语义的筛选，例如 每个工作日、节假日、每个营业日

3. relation
表示多个时间节点之间的比较或引用关系，例如 同比、环比、相比、对比。
relation 不决定 node_kind，只进入 comparison_groups。

你必须按下表选择唯一合法的 node_kind：
1. 如果节点只有 base_window，没有 modifier：
- explicit_window -> node_kind = explicit_window
- relative_window -> node_kind = relative_window
- holiday_window -> node_kind = holiday_window
- reference_window -> node_kind = reference_window
- offset_window -> node_kind = offset_window
2. 如果节点由 base_window + regular_grain 组成：
- 最终 node_kind 必须是 window_with_regular_grain
3. 如果节点由 base_window + calendar_selector 组成：
- 最终 node_kind 必须是 window_with_calendar_selector
4. 如果节点只有 regular_grain，没有明确 window：
- regular_grain 不能单独输出为一个合法 node
- 你应当优先把它并入所属的时间窗口节点
- 如果无法并入任何明确窗口，则不要构造非法 node_kind
...
6. 含 calendar_selector 的节点：本月至今每个工作日、今年节假日、某月每个营业日
...
6. node_kind 是最终合法输出类型，不是语义标签。regular_grain 和 calendar_selector 只是语义成分，不是合法 node_kind。
7. reason_code：判定原因
8. resolution_spec：供 code 使用的结构化时间语义对象，必须和 node_kind 一一对应，不能是自由文本
...
6. 如果是 window_with_regular_grain，resolution_spec 必须包含 window 和 grain。
7. 如果是 window_with_calendar_selector，resolution_spec 必须包含 window 和 selector。
8. 对 explicit_window，window_type 只允许：
{SUPPORTED_EXPLICIT_WINDOW_TYPE_LINES}
9. `named_period` 只用于一个完整自然期间；`named_period_range` 只用于月 / 季度 / 半年 / 年这种自然期间范围。
10. `named_period_range` 必须使用 `start_period` 和 `end_period`，不要自造 `range_period`、`start_month`、`end_month` 这类字段。
11. 对 calendar selector，selector_type 只允许：
{SUPPORTED_SELECTOR_TYPE_LINES}
12. reference_window 目前只允许 `alignment = same_period`。
...
7. 以下输出是非法的，绝不能生成：
   ...
   - 把 comparison / 同比 / 环比 / 相比 关系写进 resolution_spec
...
4. 根据 base_window + modifier 的组合，选择唯一合法 node_kind
5. 再填写对应的 resolution_spec
6. 最后判断 needs_clarification
7. 如果存在比较关系，再填写 comparison_groups
